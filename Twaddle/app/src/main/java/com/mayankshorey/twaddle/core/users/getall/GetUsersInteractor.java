...
public class GetUsersInteractor implements GetUsersContract.Interactor {
    ...
    public void getAllUsersFromFirebase() {
        FirebaseDatabase.getInstance().getReference().child("Campaign").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                Iterator<DataSnapshot> dataSnapshots = dataSnapshot.getChildren().iterator();
                List<Campaign> users = new ArrayList<>();

                while (dataSnapshots.hasNext()) {
                    DataSnapshot dataSnapshotChild = dataSnapshots.next();
                    Campaign user = dataSnapshotChild.getValue(Campaign.class);
                    users.add(user);
